...
{    
    ...
    .then(data => {
        ...
        for(var i = 0; i < data.days.length; i++)
        {
            if(data.days[i].day == day)
            {
                ...
                {
                    dayname.innerHTML +='<a href="'+data.days[i].subjects[j].link+'" target="_blank"><div class="period"><h2>'+data.days[i].subjects[j].name+'<br>'+data.days[i].subjects[j].time+'</h2></div></a>';
                }
                break;
            }
        }
    ...
    .then(data => {
        ...
        for(var i = 0; i < data.days.length; i++)
        {
            ...
            for(var j = 0; j < data.days[i].subjects.length; j++)
            {
                day.innerHTML +='<a href="'+data.days[i].subjects[j].link+'" target="_blank"><div class="period"><h2>'+data.days[i].subjects[j].name+'<br>'+data.days[i].subjects[j].time+'</h2></div></a>';
            }
        }
    });
